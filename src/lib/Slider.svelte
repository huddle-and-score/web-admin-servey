<script lang="ts">
	...
	export let id: null | undefined | string = null;
	export let value = parseInt('' + initialValue);
	...
	// Handles both dragging of touch/mouse and simple one-off click/touches
...
<div class="range">
	<div
		...
		tabindex="0"
	>
		<div bind:this={container} class="range__track">
			<div bind:this={progressBar} class="range__track--highlighted" />
			<div
				bind:this={thumb}
				class="range__thumb"
...
<style>
	...
	.range__wrapper:focus-visible > .range__track {
		box-shadow: 0 0 0 2px white, 0 0 0 3px #6185ff;
	}
	...
	.range__track--highlighted {
		background: #6185ff linear-gradient(90deg, #6185ff, #9c65ff);
		width: 0;
		height: 6px;
		position: absolute;
		border-radius: 999px;
	...
	.range__thumb {
		...
		user-select: none;
		box-shadow: 0 1px 1px 0 rgba(0, 0, 0, 0.14), 0 0 2px 1px rgba(0, 0, 0, 0.2);
	}

	.range__thumb--holding {
		box-shadow: 0 1px 1px 0 rgba(0, 0, 0, 0.14), 0 1px 2px 1px rgba(0, 0, 0, 0.2),
			0 0 0 6px rgba(113, 119, 250, 0.3);
	}
	...
	.range__tooltip {
		...
		border-radius: 4px;
		text-align: center;
		background: #6185ff linear-gradient(45deg, #6185ff, #9c65ff);
	}
